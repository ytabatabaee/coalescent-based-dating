...
ggsave("S100-bias_dating_n10_overall.pdf",width=3.5,height =  4)
...
ggplot(aes(x= Condition,y=l.est-l.true,color=Method), data=s[!s$Condition=='true gene trees',])+
  ...
  guides(color=guide_legend(nrow=2, byrow=TRUE),
         fill=guide_legend(nrow=2, byrow=TRUE))
#coord_cartesian(xlim=c(1,5),clip="off",ylim=c(-0.06,0.125))
ggsave("S100-bias_dating_overall_broken.pdf",width=9,height =  5)
...
ggplot(aes(x=Condition,y=abserr,color=Method),
  ...
  guides(fill=guide_legend(title="Method"))
#coord_cartesian(ylim=c(0,0.2),xlim=c(1,5),clip="off")
ggsave("S100-error-perrep_dating.pdf",width=7,height = 5)
...
ggplot(aes(color=Method, y=log10err,x=cut(AD,5)),
       data=merge(
  ...
  guides(color=guide_legend(nrow=2, byrow=TRUE),
         fill=guide_legend(nrow=2, byrow=TRUE))
ggsave("MV-logerr-perrep-ILS-bymethod_dating.pdf",width=7,height = 4.5)
...
ggplot(aes(color=Method, y=abserr,x=cut(AD,5)),
       data=merge(
         dcast(data=m[m$outgroup ==FALSE,],
  ...
  guides(color=guide_legend(nrow=2, byrow=TRUE),
         fill=guide_legend(nrow=2, byrow=TRUE))
ggsave("MV-abserr-perrep-ILS-bymethod_dating.pdf",width=7,height = 4.5)
